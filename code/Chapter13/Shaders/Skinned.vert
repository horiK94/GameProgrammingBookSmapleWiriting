...
// //増えた箇所は2と3
// //0, 1, 4は全部で(4 * 3) * 2 + 8 = 32byte, 2と3は (4 * 4) + ()  
// layout(location = 0) in vec3 inPosition;		//位置ベクトル
// layout(location = 1) in vec3 inNormal;			//法線ベクトル
// layout(location = 2) in uvec4 inSkinBones;		//4個の符号なし整数: 頂点に影響を与えるボーン
// layout(location = 3) in vec4 inSkinWeights;		//4個のfloat: 頂点に影響を与えるオーンの重み
// layout(location = 4) in vec2 inTexCoord;		//テクスチャ座標
